// Fix fn_aggregate_shift: Tính cash_amount từ payment amount thực tế thay vì grand_total
...
async function fix() {
  ...
  try {
    console.log('🔧 Fixing fn_aggregate_shift to calculate cash from actual payment amounts...\n');
    
    await client.query('BEGIN');
    
    await client.query(`DROP FUNCTION IF EXISTS fn_aggregate_shift(INT);`);
    
    // ✅ Sửa: Tính cash_amount từ order_payment.amount thực tế (ưu tiên) hoặc payment_transaction.amount
    await client.query(`
    ...
    console.log('📝 Now calculating cash from actual payment amounts (order_payment or payment_transaction)');
    console.log('📝 Instead of using settlement.grand_total\n');
    ...
    console.log(`   - Doanh thu: ${parseInt(testStats.net_amount || 0).toLocaleString('vi-VN')}đ`);
    console.log(`   - Tiền mặt: ${parseInt(testStats.cash_amount || 0).toLocaleString('vi-VN')}đ\n`);
