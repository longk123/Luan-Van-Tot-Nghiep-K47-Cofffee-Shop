/**
 * Fix v_order_money_totals để loại trừ các món đã hủy (CANCELLED)
 * Các món có trang_thai_che_bien = 'CANCELLED' không nên được tính vào tổng tiền
 */
...
async function run() {
  ...
  try {
    ...
    console.log('🔄 Fixing v_order_money_totals to exclude CANCELLED items...');
    
    // Drop old view
    await client.query(`DROP VIEW IF EXISTS v_order_money_totals CASCADE`);
    
    // Recreate view with CANCELLED exclusion
    await client.query(`
    ...
    console.log('✅ Fix complete! v_order_money_totals now excludes CANCELLED items.');
